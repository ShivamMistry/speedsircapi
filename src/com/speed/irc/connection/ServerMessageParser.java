...
public class ServerMessageParser implements Runnable {
	...
	public void run() {
		...
		while (running) {
			try {
				while (running && (s = server.getReader().readLine()) != null) {
					s = s.substring(1);
					...
					if (s.startsWith("PING")) {
						...
						if (s.contains("PRIVMSG #")) {
							...
							conversable = new ServerUser(sender, host, user,
									server);
						}
						...
						if (s.contains("NOTICE #"))
							channel = name;
						server.eventManager.fireEvent(new NoticeEvent(
								new NOTICE(msg, sender, channel), this));

					} else if (message.getCommand().equals(
							Numerics.SERVER_SUPPORT)) {
							...
							for (String t : parts) {
								if (t.startsWith("PREFIX=")) {
									...
									if (letters.length() == symbols.length()) {
										server.setModeLetters(letters
												.toCharArray());
										server.setModeSymbols(symbols
												.toCharArray());
									}
								}
							}
						...
						final Channel channel = server.channels.get(raw
								.split(" ")[2]);
						if (channel == null) {
							continue;
						}
						final ChannelUser user = channel
								.getUser(raw.split(" ")[3]);
						if (user == null) {
							continue;
						}
						...
						Channel channel = server.channels
								.get(raw.split(" ")[2]);
						if (channel == null) {
						...
						final ChannelUser u = new ChannelUser(nick, "", user,
								host, channel);
						server.getEventManager().fireEvent(
						...
						modes = modes.replace("*", "").replace("G", "")
								.replace("H", "");
						channel.userBuffer.add(new ChannelUser(nick, modes,
								user, host, channel));
						...
						if (temp[0].substring(temp[0].indexOf("TOPIC"))
						...
						for (Channel channel : server.channels.values()) {
							final ChannelUser user = channel.getUser(message
									.getSender().split("!")[0]);
							if (user != null) {
								user.setNick(raw.substring(
										raw.indexOf(": ") + 2).trim());
							}
						}
					}
					server.eventManager.fireEvent(new RawMessageEvent(message,
							this));
